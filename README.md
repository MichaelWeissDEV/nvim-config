...
- **Broad language coverage**, from systems languages (C/C++/Rust/Go/Zig/
  ...
  documentation (Markdown/LaTeX/reStructuredText). Full matrix: [LANGUAGES.md](LANGUAGES.md).
...
Full profile list and every tool each one installs: [TOOLS.md](TOOLS.md). This
...
Full table, generated from the config's own keymap registry: [KEYMAPS.md](KEYMAPS.md).
Interactive: press `<space>` for which-key, or run `:NvimKeymaps`.

## Commands

Full generated list: [COMMANDS.md](COMMANDS.md), or run `:NvimCommands` inside Neovim.
...
This README is the entry point; details live in the local Sphinx site and
in a handful of generated Markdown files at the repo root:
...
- **Native `:help nvim-config`** — a fast, always-available subset,
  ...
  itself reads.
- **[KEYMAPS.md](KEYMAPS.md)**, **[COMMANDS.md](COMMANDS.md)**,
  **[PLUGINS.md](PLUGINS.md)**, **[TOOLS.md](TOOLS.md)**,
  **[LANGUAGES.md](LANGUAGES.md)** — generated by
  `scripts/generate-docs.lua` from the config's own registries, so these
  can't drift from what's actually configured. Regenerate with
...
language: [LANGUAGES.md](LANGUAGES.md).
